...
def respond_config():
  if request.form['url'] != 'undefined':
    ...
    try:
      data = pd.read_parquet(BytesIO(db_entry['filtered_dataframe'])).to_json(orient='index')
    except:
      if type(db_entry['transformed_dataframe']) == bytes: # The mockup db_entry stores the empty transformed_dataframe as a list, so don't convert that one.
        data = pd.read_parquet(BytesIO(db_entry['transformed_dataframe'])).to_json(orient='index')
      else:
